...
async def execute_action(env: HarnessEnv, action: Action) -> RawState:
    match action:
        ...
        case ReadFileAction(path=path, start_line=start_line, end_line=end_line):
            ...
            # No end_line => read to EOF (sed's `$`); the per-observation char
            # limit, not a baked line window, bounds how much the agent sees.
...
def build_system_prompt() -> str:
    return "\n".join(
        [
            ...
            "Call verify to receive the authoritative task judgment; a verify ends the trial.",
        ]
    )
